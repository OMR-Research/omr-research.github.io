...
"""
import argparse, re, sys, html as html_mod
import subprocess
...
def parse_bib_text(text):
    ...
    for m in re.finditer(r'@(\w+)\s*[\{(]', text):
        ...
        if etype.lower() in ("comment","string","preamble"):
            continue
        start = m.start(); depth = 0; pos = m.end()-1; entry_end = len(text)
        while pos < len(text):
            if text[pos] in "{(": depth += 1
            elif text[pos] in "})":
                depth -= 1
                if depth == 0: entry_end = pos+1; break
            pos += 1
...
def _parse_entry(txt, etype):
    ...
    while pos < len(txt):
        ...
        if pos >= len(txt) or txt[pos] in "})": break
        fm = re.match(r"(\w+)\s*=\s*", txt[pos:])
        if not fm: pos += 1; continue
        fname = fm.group(1).lower(); pos += fm.end()
        if pos >= len(txt): break
        if txt[pos] == "{":
            depth=0; vstart=pos+1
            while pos < len(txt):
                if txt[pos]=="{": depth+=1
                elif txt[pos]=="}":
                    depth-=1
                    if depth==0: fval=txt[vstart:pos]; pos+=1; break
                pos+=1
            else: fval=txt[vstart:]
        elif txt[pos] == '"':
            vstart=pos+1; pos+=1
            while pos < len(txt) and txt[pos]!='"': pos+=1
            fval=txt[vstart:pos]; pos+=1
        else:
            vstart=pos
        ...
        while pos < len(txt) and txt[pos] in " \t\n\r": pos+=1
        if pos < len(txt) and txt[pos]==",": pos+=1
    return fields
...
_AMAP    = {"'":_ACUTE,"`":_GRAVE,"^":_CIRC,'"':_UML,"~":_TILDE,
            "v":_CARON,"c":_CED,"u":_BREVE,"=":_MACRON,"H":_HUNGDBL,
            ".":_DOT,"k":_OGONEK}

def _sa(cmd, ch): return _AMAP.get(cmd, {}).get(ch, ch)
...
def l2u(s):
    ...
    s = re.sub(r"\{\\`\{?\\i\}?\}", "ì", s)
    CMDS = r"'`\^\"~vcuH=\.k"
    s = re.sub(r"\{\\(["+CMDS+r"])\{([a-zA-Z])\}\}", lambda m: _sa(m.group(1), m.group(2)), s)
    s = re.sub(r"\\(["+CMDS+r"])\{([a-zA-Z])\}",     lambda m: _sa(m.group(1), m.group(2)), s)
    s = re.sub(r"\{\\(["+CMDS+r"])([a-zA-Z])\}",     lambda m: _sa(m.group(1), m.group(2)), s)
    s = re.sub(r"\\(["+CMDS+r"])([a-zA-Z])",          lambda m: _sa(m.group(1), m.group(2)), s)
    for p, r in [
        ...
        (r"\{?\\oe\}?", "œ"),  (r"\{?\\OE\}?", "Œ"),
        (r"\\&", "&"), (r"\\%", "%"), (r"\\_", "_"), (r"\\#", "#"),
        (r"\\textendash\b", "–"), (r"\\textemdash\b", "—"), (r"\\ldots\b", "…"),
        (r"\\url\{[^}]*\}", ""),
        (r"\\href\{[^}]*\}\{([^}]*)\}", r"\1"),
    ]:
        s = re.sub(p, r, s)
...
def compare(base_map, curr_map):
    ...
    for key, ce in curr_map.items():
        ...
        for f in sorted(all_fields):
            if f in IGNORE_FIELDS or f in ("entrytype","id"): continue
            bv = be.get(f,"").strip()
            cv = ce.get(f,"").strip()
            if bv == cv: continue
            if f == "abstract":
                abs_diff = True; abs_row = (bv, cv)
            elif f == "file":
                pdf_diff = True; pdf_rows = (bv, cv)
            elif f in DOI_URL_FIELDS:
                doi_diff = True; doi_rows = (f, bv, cv)
            else:
                meta_diff = True

        if abs_diff:   abstract_added.append((ce, abs_row))
        if pdf_diff:   pdf_changed.append((ce, pdf_rows))
        if doi_diff:   link_changed.append((ce, doi_rows))
...
def render_new(entries):
    ...
    for e in entries:
        ...
        out += '<table class="diff-table"><tbody>' + rows_from_entry(e) + '</tbody></table></div>'
    return out
...
def render_pdf_changed(entries):
    ...
    for e, (bv, cv) in entries:
        out += '<div class="entry">' + entry_head(e)
        out += '<table class="diff-table"><tbody>'
        out += f'<tr class="changed"><td class="field">file</td>'
...
def main():
    ...
    if result.returncode != 0:
        print(f"ERROR: could not read OMR-Research.bib from commit {baseline!r}", file=sys.stderr)
        print(result.stderr, file=sys.stderr)
        sys.exit(1)

    base_entries = parse_bib_text(result.stdout)
    curr_entries = parse_bib_text((BASE / "OMR-Research.bib").read_text(encoding="utf-8"))

    base_map = {e["ID"]: e for e in base_entries}
    curr_map = {e["ID"]: e for e in curr_entries}

    new_entries, abstract_added, pdf_changed, link_changed, meta_changed = compare(base_map, curr_map)

    total_changed = len(abstract_added)+len(pdf_changed)+len(link_changed)+len(meta_changed)

    toc_items = []
    if new_entries:    toc_items.append(f'<li><a href="#sec-new">New entries: {len(new_entries)}</a></li>')
    if abstract_added: toc_items.append(f'<li><a href="#sec-abstract">Abstract added/changed: {len(abstract_added)}</a></li>')
    if pdf_changed:    toc_items.append(f'<li><a href="#sec-pdf">PDF link added/changed: {len(pdf_changed)}</a></li>')
    if link_changed:   toc_items.append(f'<li><a href="#sec-doi">DOI/URL changed: {len(link_changed)}</a></li>')
    if meta_changed:   toc_items.append(f'<li><a href="#sec-meta">Other metadata: {len(meta_changed)}</a></li>')

    body  = section("sec-new",      "NEW ENTRIES",             "#1a7a4a", len(new_entries),    render_new(new_entries))
    body += section("sec-abstract", "ABSTRACT ADDED/CHANGED",  "#155799", len(abstract_added), render_abstract_added(abstract_added))
    body += section("sec-pdf",      "PDF LINK ADDED/CHANGED",  "#6c757d", len(pdf_changed),    render_pdf_changed(pdf_changed))
    body += section("sec-doi",      "DOI/URL CHANGED",         "#6c4000", len(link_changed),   render_link_changed(link_changed))
    body += section("sec-meta",     "OTHER METADATA CHANGED",  "#555",    len(meta_changed),   render_meta(meta_changed))
...
<main>{body}</main>
    ...
    print(f"Written {OUT}")
    print(f"  New: {len(new_entries)}, Abstract: {len(abstract_added)}, PDF: {len(pdf_changed)}, DOI/URL: {len(link_changed)}, Meta: {len(meta_changed)}")
